...
public class WOEventDisplayPage extends WOEventPage {
	...
	public WOEventDisplayPage(WOContext aContext) {
		...
		// we sort these things descending, i.e. the opposite of normal sorting
		// order
	...
	public NSArray filterEvents(NSArray evs, int level) {
		...
		// by their plain duration, which is what the default implementation
		// does.
		try {
	...
	public long durationOfEvent(EOEvent e) {
		...
		// mode 4 is in so far special as we need to filter out events which
		// will not
