...
public class ERXArrayUtilities {
	...
	/**
	 * Holds the null grouping key for use when grouping objects based on a key
	 * that might return null and nulls are allowed
	 */
	...
	/**
	 * The qualifiers EOSortOrdering.CompareAscending and friends are actually
	 * 'special' and processed in a different/faster way when sorting than a
	 * selector that would be created by:
	 * <code>new NSSelector("compareAscending", ObjectClassArray)</code> This
	 * method eases the pain on creating those selectors from a string.
	 *
	 * @param key
	 *            sort key
	 ...
	 * in a dictionary. If passed a null array, null is returned. If passed a
	 * null keyPath, an empty dictionary is returned.
	 * <p>
	 * If one starts with:
	 * 
	 * <pre>
	 * <code>( { lastName = "Barker"; firstName = "Bob"; favoriteColor = "blue"; },
	{ firstName = "Bob"; favoriteColor = "red"; },
	{ lastName = "Further"; firstName = "Frank"; favoriteColor = "green"; } )</code>
	 * </pre>
	 * 
	 * and one calls <code>arrayGroupedByKeyPath(objects, "firstName")</code>,
	 * one gets:
	 * 
	 * <pre>
	 * <code>{ "Bob" = ( { lastName = "Barker"; firstName = "Bob"; favoriteColor = "blue"; }, { firstName = "Bob"; favoriteColor = "red"; } );
	"Frank" = ( { lastName = "Further"; firstName = "Frank"; favoriteColor = "green"; } ); }</code>
	 * </pre>
	 * 
	 * If one calls <code>arrayGroupedByKeyPath(objects, "lastName")</code>, one
	 * gets:
	 * 
	 * <pre>
	 * <code>{ "Bob" = ( { lastName = "Barker"; firstName = "Bob"; favoriteColor = "blue"; } );
	"Frank" = ( { lastName = "Further"; firstName = "Frank"; favoriteColor = "green"; } );
	"**** NULL GROUPING KEY ****" = ( { firstName = "Bob"; favoriteColor = "red"; } ); }</code>
	 * </pre>
	 * <p>
	 * This method calls
	 * <code>arrayGroupedByKeyPath(objects, keyPath, includeNulls, valueKeyPath)</code>
	 * with includeNulls set to true and valueKeyPath set to null.
	 * 
	 * @param objects
	 *            array of objects to be grouped
	 * @param keyPath
	 *            path into objects used to group the objects
	 * @return a dictionary where the keys are the grouped values and the
	 *         objects are arrays of the objects that have that value. Objects
	 *         for which the key path returns null will be grouped with the key
	 *         {@link #NULL_GROUPING_KEY}
	 */
	 ...
	 * contents of the array, using the result of the valueForKey call as a key
	 * in a dictionary. If passed a null array, null is returned. If passed a
	 * null keyPath, an empty dictionary is returned. If valueKeyPath is not
	 * null, then the grouped arrays each have valueForKey called with
	 * valueKeyPath and the grouped arrays are replaced with the results of
	 * those calls.
	 * <p>
	 * If one starts with:
	 * 
	 ...
	 * 
	 * and one calls
	 * <code>arrayGroupedByKeyPath(objects, "firstName", true, "favoriteColor")</code>,
	 * one gets:
	 * 
	 * <pre>
	 * <code>{Frank = ("green"); Bob = ("blue", "red");</code>
	 * </pre>
	 * 
	 * If one calls
	 * <code>arrayGroupedByKeyPath(objects, "lastName", false, "favoriteColor")</code>,
	 * one gets:
	 * 
	 * <pre>
	 * <code>{Further = ("green"); Barker = ("blue"); }</code>
	 * </pre>
	 * 
	 * If one calls
	 * <code>arrayGroupedByKeyPath(objects, "lastName", true, "favoriteColor")</code>,
	 * one gets:
	 * 
	 * <pre>
	 * <code>{Further = ("green"); Barker = ("blue"); "**** NULL GROUPING KEY ****" = ("red"); }</code>
	 * </pre>
	 *
	 * @param objects
	 *            array of objects to be grouped
	 ...
	 *            path into objects used to group the objects
	 * @param includeNulls
	 *            determines if keyPaths that resolve to null are included in
	 *            the resulting dictionary
	 * @param valueKeyPath
	 *            used to call valueForKey on the arrays in the results
	 *            dictionary, with the results of those calls each replacing the
	 *            corresponding array in the results dictionary.
	 * @return a dictionary where the keys are the grouped values and the
	 *         objects are arrays of the objects that have that value. Objects
	 *         for which the key path returns null will be grouped with the key
	 *         {@link #NULL_GROUPING_KEY}
	 ...
	 * null keyPath, an empty dictionary is returned. If valueKeyPath is not
	 * null, then the grouped arrays each have valueForKey called with
	 * valueKeyPath and the grouped arrays are replaced with the results of that
	 * call.
	 * <p>
	 * If one starts with:
	 * 
	 ...
	 * 
	 * and one calls
	 * <code>arrayGroupedByKeyPath(objects, "firstName", null, "favoriteColor")</code>,
	 * one gets:
	 * 
	 * <pre>
	 * <code>{Frank = ("green"); Bob = ("blue", "red");</code>
	 ...
	 * 
	 * If one calls
	 * <code>arrayGroupedByKeyPath(objects, "lastName", "extra", "favoriteColor")</code>,
	 * one gets:
	 * 
	 * <pre>
	 * <code>{Further = ("green"); Barker = ("blue"); "extra" = ("red"); }</code>
	 * </pre>
	 * 
	 * If one calls
	 * <code>arrayGroupedByKeyPath(objects, "lastName", null, "favoriteColor")</code>,
	 * one gets:
	 * 
	 * <pre>
	 * <code>{Further = ("green"); Barker = ("blue"); "**** NULL GROUPING KEY ****" = ("red"); }</code>
	 ...
	 * @param keyPath
	 *            path into objects used to group the objects
	 * @param nullGroupingKey
	 *            used as the key in the results dictionary for the array of
	 *            objects for which the valueForKey with keyPath result is null.
	 * @param valueKeyPath
	 *            used to call valueForKey on the arrays in the results
	 *            dictionary, with the results of those calls each replacing the
	 *            corresponding array in the results dictionary.
	 ...
	 * single item in the to-many will put the object in the corresponding
	 * group. A typical example is an array of users with a roles relationship.
	 * The result to calling
	 * <code>arrayGroupedByToManyKeyPath(users, "roles.name")</code> would be
	 * <code>"admin" = (user1, user2); "editor" = (user3);...</code>. The
	 * dictionary that is returned contains keys that correspond to the grouped
	 * keys values. This means that the object pointed to by the key path must
	 * be a cloneable object. For instance using the key path 'users' would not
	 * work because enterprise objects are not cloneable. Instead you might
	 * choose to use the key path 'users.name' of 'users.primaryKey', if your
	 * enterprise objects support this see {@link ERXGenericRecord} if
	 * interested.
	 * 
	 * @param objects
	 *            array of objects to be grouped
	 ...
	 * @param includeNulls
	 *            determines if keyPaths that resolve to null should be allowed
	 *            into the group.
	 * @return a dictionary where the keys are the grouped values and the
	 *         objects are arrays of the objects that have the grouped
	 *         characteristic. Note that if the key path returns null then one
	 *         of the keys will be the static ivar NULL_GROUPING_KEY
	 */
	 ...
	 * Groups an array of objects by a given to-many key path, where every
	 * single item in the to-many will put the object in the corresponding
	 * group. The dictionary that is returned contains keys that correspond to
	 * the grouped keys values. This means that the object pointed to by the key
	 * path must be a cloneable object. For instance using the key path 'users'
	 * would not work because enterprise objects are not cloneable. Instead you
	 * might choose to use the key path 'users.name' of 'users.primaryKey', if
	 * your enterprise objects support this see {@link ERXGenericRecord} if
	 * interested.
	 * 
	 * @param objects
	 *            array of objects to be grouped
	 ...
	 *            should be allowed into the group; if so, this key is used for
	 *            them
	 * @return a dictionary where the keys are the grouped values and the
	 *         objects are arrays of the objects that have the grouped
	 *         characteristic. Note that if the key path returns null then one
	 ...
	 *            them
	 * @param valueKeyPath
	 *            allows the grouped objects in the result to be derived from
	 *            objects (by evaluating valueKeyPath), instead of being members
	 *            of the objects collection. Objects that evaluate valueKeyPath
	 *            to null have no value included in the result
	 * @return a dictionary where the keys are the grouped values and the
	 *         objects are arrays of the objects that have the grouped
	 *         characteristic. Note that if the key path returns null then one
	 *         of the keys will be the static ivar NULL_GROUPING_KEY
	...
	public static <T> NSDictionary arrayGroupedByToManyKeyPath(Collection<T> objects,
		...
		for (T object : objects) {
			...
			if (!isNullKey || nullGroupingKey != null) {
				...
				Enumeration keys = array.objectEnumerator(); keys.hasMoreElements();) {
	 ...
	 * interface such as {@link com.webobjects.foundation.NSArray NSArray},
	 * {@link com.webobjects.foundation.NSSet NSSet}, {@link Vector} and
	 * {@link Hashtable} using the
	 * {@link com.webobjects.eocontrol.EOQualifierEvaluation
	 * EOQualifierEvaluation} interface.
	 *
	 * @param enumeration
	 *            to be filtered; to obtain an enumeration, use
	 *            objectEnumerator() for the collections in
	 *            com.webobjects.foundation package and use elements() for the
	 *            Vector and Hashtable
	 * @param qualifier
	 *            to do the filtering
	 * @return array of filtered results.
	 * @deprecated use
	 *             {@link #filteredArrayWithQualifierEvaluation(Iterable, EOQualifierEvaluation)}
	 *             instead
	 */
	@Deprecated
	...
	/**
	 * Checks if the given enumeration contains at least one match defined by
	 * the given object implementing the
	 * {@link com.webobjects.eocontrol.EOQualifierEvaluation
	 * EOQualifierEvaluation} interface.
	 *
	 * @param enumeration
	...
	/**
	 * Checks if the given iterator contains at least one match defined by the
	 * given object implementing the
	 * {@link com.webobjects.eocontrol.EOQualifierEvaluation
	 * EOQualifierEvaluation} interface.
	 *
	 * @param iterator
	 ...
	 * and {@link TreeSet} using the
	 * {@link com.webobjects.eocontrol.EOQualifierEvaluation
	 * EOQualifierEvaluation} interface.
	 *
	 * @param iterator
	 *            to be filtered; use iterator() to obtain an iterator from the
	 *            collections
	 * @param qualifier
	 *            to do the filtering
	 * @return array of filtered results.
	...
	/**
	 * Filters out duplicates of an array of objects based on the value of the
	 * given key path off of those objects. Objects with a null value will be
	 * skipped, too.
	 * 
	 * @param <T>
	 *            class of array items
	 ...
	 *            array of objects
	 * @param keyPath
	 *            key path to be evaluated off of every object
	...
	/**
	 * Creates an array preserving order by adding all of the non-duplicate
	 * values from the second array to the first.
	 * 
	 * @param <T>
	 *            class of array items
	 * @param array1
	 ...
	 * @param array2
	 *            second array
	 * @return array containing all of the elements of the first array and all
	 *         of the non-duplicate elements of the second array.
	 */
	 ...
	 * @return an array containing all objects but the first of the parameter
	 *         array. if null is passed, null is returned. if the parameter
	 *         array is empty, an empty array is returned.
	 */
	 ...
	 *            class of array items
	 * @param array
	 *            mutable array where non-null object will be added
	 * @param object
	...
	/**
	 * Adds all of the non-duplicate elements from the second array to the
	 * mutable array.
	 * 
	 * @param <T>
	 *            class of array items
	 * @param array1
	 *            mutable array where non-duplicate objects are added
	 * @param array2
	 *            array to be added to a1
	 */
	...
	/**
	 * Recursively flattens an array of arrays and individual objects into a
	 * single array of elements.
	 * <p>
	 * For example:
	 * 
	 * <pre>
	 * <code>NSArray foos; //Assume exists
	 * NSArray bars = (NSArray)foos.valueForKey("toBars");
	 * </code>
	 * </pre>
	 * 
	 * In this case if <code>foos</code> contained five elements then the array
	 * <code>bars</code> will contain five arrays each corresponding to what
	 * <code>aFoo.toBars</code> would return. To have the entire collection of
	 * <code>bars</code> in one single array you would call:
	 * <code>NSArray allBars = flatten(bars)</code>
	 * 
	 * @param array
	 *            array to be flattened
	 * @param filterDuplicates
	 *            determines if the duplicate values should be filtered
	 * @return an array containing all of the elements from all of the arrays
	 *         contained within the array passed in. (Optionally, with duplicate
	 ...
	 *            array to be flattened
	 * @return an array containing all of the elements from all of the arrays
	 *         contained within the array passed in.
	 */
	@SuppressWarnings("unchecked")
	public static NSArray flatten(NSArray<?> array) {
		...
		NSMutableArray<Object> result = null; // Not gonna create a new array if
		...
		for (int i = 0; i < array.size(); i++) {
			...
			if (element instanceof NSArray) {
				...
				result.add(element);
			} // Otherwise let's put off copying the element, the backfill
				// section above will take care of it.
		}

		// CLEANUP: Arguably safer to return the immutable array we are declared
		// as returning
		return result != null ? result : array;
	}

	/**
	 * Creates an NSArray from a resource associated with a given bundle that is
	 ...
	 * 
	 * @param source
	 *            collection or object to be acted upon.
	 * @param keyPaths
	 *            array of key paths.
	 * @return for collections, returns an array containing an array of values
	 *         for every key path. For objects, returns an array containing a
	 *         value for every key path.
	 */
	...
	/**
	 * Returns the first object of the array. If the array is null or empty,
	 * null is returned.
	 * 
	 * @param <T>
	 *            class of array items
	...
	/**
	 * Finds the index of the first object in the array with a given value for a
	 * given key path. Assumes that all objects in the array either are
	 * NSKeyValueCoding.NullValue or have the given key path.
	 * 
	 * @param <T>
	 *            class of array items
	 * @param array
	 ...
	 * @param keyPath
	 *            the key path to use to compare to value
	 * @return index of the first object with the qualification. -1 if none
	 *         matches.
	 */
	...
	/**
	 * Finds the first object in the array with a given value for a given key
	 * path.
	 * 
	 * @param <T>
	 *            class of array items
	 ...
	 * @param keyPath
	 *            the key path to use to compare to value
	 * @return first object in the array with the qualification. null if none
	 *         matches.
	 */
	...
	/**
	 * Walks over an array and returns an array of objects from that array that
	 * have a particular value for a particular key path. Treats null and
	 * NSKeyValueCoding.NullValue equivalently. Any NSKeyValueCoding.NullValue
	 * objects in the array are skipped. If array is null or empty, an empty
	 * array is returned.
	 * 
	 * @param <T>
	 *            class of array items
	 ...
	 *            value to look for
	 * @param keyPath
	 *            key path to apply on each object on the array to compare
	 *            against valueToLookFor
	...
	/**
	 * Sorts a given array with a key in ascending fashion and returns a mutable
	 * clone of the result.
	 * 
	 * @param array
	 *            array to be sorted.
	 ...
	 * @param selector
	 *            sort order selector to use, if null, then sort will be case
	 *            insensitive ascending. a.addObject(theObject);
	 ...
	 *            sort key.
	 * @param selector
	 *            sort order selector to use, if null, then sort will be
	 *            ascending.
	 */
	 ...
	 * {@link com.webobjects.foundation.NSArray.Operator NSArray.Operator}. This
	 * class adds support for chaining multiple array operators in a single
	 * keypath via its
	 * {@link er.extensions.foundation.ERXArrayUtilities.BaseOperator#contents(NSArray, String)
	 * contents} method.
	 */
	static abstract class BaseOperator implements NSArray.Operator {
		 ...
		 * {@link com.webobjects.foundation.NSKeyValueCodingAdditions.Utility#valueForKeyPath(Object, String)
		 * valueForKeyPath} on each array object, this method operates by
		 * calling
		 * {@link com.webobjects.foundation.NSArray#valueForKeyPath(String)
		 * valueForKeyPath} on the array argument instead. This method is used
		 * by Wonder operators to chain multiple array operators in a single key
		 * path.
		 * 
		 * @param array
		 ...
		 * @return the object value produced by valueForKeyPath, or the array
		 *         itself if the keypath is empty
		 */
	...
	/**
	 * Define an {@link com.webobjects.foundation.NSArray.Operator
	 * NSArray.Operator} for the key <b>sort</b>.
	 * <p>
	 ...
	 * </ol>
	 * Which in the first case would return myArray sorted ascending by first
	 * name and the second case by lastName and then by the length() of the
	 * firstName. Due to the way the sort key arguments are written, this key
	 * cannot occur anywhere except at the very end of the keypath.
	 */
	...
	/**
	 * Define an {@link com.webobjects.foundation.NSArray.Operator
	 * NSArray.Operator} for the key <b>flatten</b>.
	 * <p>
	 * This allows for key value paths like:
	 * 
	 * <pre>
	 * <code>myArray.valueForKey("@flatten.someOtherPath");</code>
	 * </pre>
	 * 
	 * Which in this case would return myArray flattened if myArray is an
	 * NSArray of NSArrays (of NSArrays etc) before continuing to process
	 * someOtherPath.
	 * 
	 * @see BaseOperator
	 */
	...
	/**
	 * Define an {@link com.webobjects.foundation.NSArray.Operator
	 * NSArray.Operator} for the key <b>isEmpty</b>.
	 * <p>
	 * This allows for key value paths like:
	 * 
	 * <pre>
	 * <code>myArray.valueForKey("@isEmpty");</code>
	 * </pre>
	 * 
	 * Which in this case would return {@link java.lang.Boolean#TRUE true} if
	 * the myArray.count() == 0, or {@link java.lang.Boolean#FALSE false} if it
	 * is not. This operator always ends computation. Any keypath following the
	 * isEmpty operator is simply ignored.
	 * 
	 */
	public static class IsEmptyOperator implements NSArray.Operator {
		...
		/**
		 * returns true if the given array is empty, useful for WOHyperlink
		 * disabled binding.
		 * 
		 * @param array
		 *            array to be checked.
		 ...
		 * @return <code>Boolean.TRUE</code> if array is empty,
	...
	/**
	 * Define an {@link com.webobjects.foundation.NSArray.Operator
	 * NSArray.Operator} for the key <b>subarrayWithRange</b>.
	 * <p>
	 * This allows for key value paths like:
	 * 
	 * <pre>
	 * <code>myArray.valueForKeyPath("@subarrayWithRange.20-3.someOtherPath");</code>
	 * </pre>
	 * 
	 * Which in this case would return the three objects from
	 * <code>myArray</code>, starting at the index of 20, before continuing to
	 * process <code>someOtherPath</code>.
	 * <p>
	 * Note that the syntax for the range argument is <b>not</b>
	 * startIndex-endIndex. The API matches that of NSRange. You must provide a
	 * start index and an array length.
	 * 
	 * @see BaseOperator
	 */
	...
	/**
	 * Define an {@link com.webobjects.foundation.NSArray.Operator
	 * NSArray.Operator} for the key <b>limit</b>, which is similar to
	 * subarrayWithRange except it is always from 0 to the limit value. If the
	 * limit specified is larger than the size of the array, the entire array
	 * will be returned.
	 * <p>
	 * This allows for key value paths like:
	 * 
	 * <pre>
	 * <code>myArray.valueForKeyPath("@limit.10.someOtherPath");</code>
	 * </pre>
	 * 
	 * Which in this case would return the first 10 objects in
	 * <code>myArray</code> before continuing to process
	 * <code>someOtherPath</code>.
	 * 
	 * @see BaseOperator
	 */
	...
	/**
	 * Define an {@link com.webobjects.foundation.NSArray.Operator
	 * NSArray.Operator} for the key <b>unique</b>.
	 * <p>
	 * This allows for key value paths like:
	 * 
	 * <pre>
	 * <code>myArray.valueForKeyPath("@unique.someOtherPath");</code>
	 * </pre>
	 * 
	 * Which in this case would return only those objects which are unique in
	 * myArray before continuing to process someOtherPath.
	 * 
	 * @see BaseOperator
	 */
	public static class UniqueOperator extends BaseOperator {
		 ...
		 * @return the value produced by following the keypath after removing
		 *         duplicates
		 */
		public Object compute(NSArray array, String keypath) {
			if (array != null)
	...
	/**
	 * Define an {@link com.webobjects.foundation.NSArray.Operator
	 * NSArray.Operator} for the key <b>removeNullValues</b>.
	 * <p>
	 * This allows for key value paths like:
	 * 
	 * <pre>
	 * <code>myArray.valueForKeyPath("@removeNullValues.someOtherPath");</code>
	 * </pre>
	 * 
	 * Which in this case would remove the occurrences of NSKeyValueCoding.Null
	 * from myArray before continuing to process someOtherPath.
	 * 
	 * @see BaseOperator
	 */
	public static class RemoveNullValuesOperator extends BaseOperator {
		 ...
		 * @return the value produced by following keypath after filtering nulls
		 *         from the array
		 */
		public Object compute(NSArray array, String keypath) {
	...
	/**
	 * Define an {@link com.webobjects.foundation.NSArray.Operator
	 * NSArray.Operator} for the key <b>objectAtIndex</b>.
	 * <p>
	 * This allows for key value paths like:
	 * 
	 * <pre>
	 * <code>myArray.valueForKey("@objectAtIndex.3.firstName");</code>
	 * </pre>
	 */
	public static class ObjectAtIndexOperator implements NSArray.Operator {
		 ...
		 *            array to be checked.
		 * @param keypath
		 *            integer value of index (zero based).
		 * @return <code>null</code> if array is empty or value is not in index,
		 *         <code>keypath</code> value for the object at index otherwise.
		 */
		public Object compute(NSArray array, String keypath) {
	...
	/**
	 * Define an {@link com.webobjects.foundation.NSArray.Operator
	 * NSArray.Operator} for the key <b>avgNonNull</b>.
	 * <p>
	 * This allows for key value paths like:
	 * <ul>
	 ...
	 * of the keypath to its right. It should not be followed by an array or any
	 * other array operators. This is because it does not call
	 * {@link com.webobjects.foundation.NSArray#valueForKeyPath(String)
	 * valueForKeyPath} on the array to its left, but instead loops through the
	 * values of the array to its left, calling
	 * {@link com.webobjects.foundation.NSKeyValueCodingAdditions.Utility#valueForKeyPath(Object, String)
	 * valueForKeyPath} on the individual array values instead. This behavior is
	 * consistent with Apple's standard NSArray operators.
	 */
	...
	/**
	 * Define an {@link com.webobjects.foundation.NSArray.Operator
	 * NSArray.Operator} for the key <b>reverse</b>.
	 * <p>
	 * This allows for key value paths like:
	 * 
	 * <pre>
	 * <code>myArray.valueForKey("@reverse.someOtherPath");</code>
	 * </pre>
	 * 
	...
	/**
	 * Define an {@link com.webobjects.foundation.NSArray.Operator
	 * NSArray.Operator} for the key <b>median</b>.
	 * <p>
	 * This allows for key value paths like:
	 * <ul>
	 ...
	 * <p>
	 * The @median operator applies to the array of objects to its left if it is
	 * the last key in the path. Otherwise it applies to the end of the keypath
	 * to its right. It should not be followed by an array or any other array
	 * operators. This is because it does not call
	 * {@link com.webobjects.foundation.NSArray#valueForKeyPath(String)
	 * valueForKeyPath} on the array to its left, but instead loops through the
	 * values of the array to its left, calling
	 * {@link com.webobjects.foundation.NSKeyValueCodingAdditions.Utility#valueForKeyPath(Object, String)
	...
	/**
	 * Define an {@link com.webobjects.foundation.NSArray.Operator
	 * NSArray.Operator} for the key <b>stdDev</b> and <b>popStdDev</b>.
	 * <p>
	 * This allows for key value paths like:
	 * <ul>
	 ...
	 * </ul>
	 * All three of these examples will return the same value, which in this
	 * case is the standard deviation of the amounts. The standard deviation is
	 * a measure of the dispersion of a sample of numbers. The population
	 * standard deviation is used if you have the values for an entire
	 * population.
	 * <p>
	...
	/**
	 * Will register new NSArray operators <b>sort</b>, <b>sortAsc</b>,
	 * <b>sortDesc</b>, <b>sortInsensitiveAsc</b>, <b>sortInsensitiveDesc</b>,
	 * <b>unique</b>, <b>flatten</b>, <b>reverse</b>, <b>limit</b>, and
	 * <b>fetchSpec</b>
	 */
	...
	/**
	 * Calculates the median value of an array. The median is the value for
	 * which half of the elements are above and half the elements are below. As
	 * such, an array sort is needed and this might be very costly depending of
	 ...
	 * the specified keypath. If the keypath is null or empty, then the array
	 * values are used instead. If the array has fewer than two objects, null is
	 * returned. If isPopulation is true, the population standard deviation is
	 * calculated. If isPopulation is false, the sample standard deviation is
	 * calculated. Use a true value for isPopulation if you know the values for
	 * an entire population and false if you are dealing with a sample.
	 * 
	 * @param array
	 *            an array of objects
	 * @param keypath
	 *            a key path to a numeric value on each object
	 * @param isPopulation
	...
	/**
	 * Shorter name for arrayWithoutDuplicates, which I always forget the name
	 * of.
	 * 
	 * @param <T>
	 *            class of array items
	...
	/**
	 * Filters out all of the duplicate objects in a given array. Preserves the
	 * order now.
	 * 
	 * @param <T>
	 *            class of array items
	...
	/**
	 * Function to determine if an array contains any of the elements of another
	 * array.
	 * 
	 * @param <T>
	 *            class of array items
	 * @param array1
	 *            to test if it contains any of the objects
	 * @param array2
	 *            array of objects to test if the first array contains any of
	 * @return if the first array contains any elements from the second array
	 */
	...
	/**
	 * Function to determine if an array contains all of the elements of another
	 * array.
	 * 
	 * @param <T>
	 *            class of array items
	 * @param array1
	 *            to test if it contains all of the objects of another array
	 * @param array2
	 *            array of objects to test if the first array contains all of
	 ...
	 * <p>
	 * For example, given an array containing three user objects and the
	 * attribute key "firstName", the result of calling this method would be the
	 * string: "Max, Anjo and Patrice".
	 * 
	 * @param list
	 *            of objects to be displayed in a friendly manner
	 * @param attribute
	 *            key to be called on each object in the list
	 * @param nullArrayDisplay
	 *            string to be returned if the list is null or empty
	 * @param separator
	 *            string to be used for the first items
	...
	/**
	 * Returns an array of dictionaries containing the key/value pairs for the
	 * given paths.
	 * 
	 * @param array
	 *            array of objects
	...
	/**
	 * Removes all occurrences of NSKeyValueCoding.NullValue in the provided
	 * array
	 * 
	 * @param array
	...
	/**
	 * Converts an Object array to a String array by casting each element. This
	 * is analogous to
	 * <code>String[] myStringArray = (String[])myObjectArray;</code> except
	 * that it creates a clone of the array.
	 * 
	 * @param o
	...
	/**
	 * pretty prints a two dimensional Object array which is ugly when using
	 * toString
	 * 
	 * @param array
	 *            the object which one wants to print as a String
	...
	/**
	 * pretty prints a NSArray of two dimensional Object array which is ugly
	 * when using toString
	 * 
	 * @param array
	 *            the object which one wants to print as a String
	...
	/**
	 * removes all occurrences of NSKeyValueCoding.Null from the end of the
	 * array
	 * 
	 * @param array
	 *            the array from which the values should be removed
	 * @return a new NSArray which does not have NSKeyValueCoding.Null instances
	 *         at the end
	 */
	...
	/**
	 * Given an array of objects, returns a dictionary mapping the value by
	 * performing valueForKeyPath on each object in the array to the object in
	 * the array. This method assume that the value returned for the keyPath
	 * attribute will be unique for all the objects in the array. In case of
	 * duplicate entry, the new object will replace the previous one in the
	 * dictionary.
	 * <p>
	 * Calls <code>dictionaryOfObjectsIndexedByKeyPathThrowOnCollision()</code>
	 * passing <code>false</code> for throwOnCollision.
	 * 
	 * @param <K>
	 *            class of key path value
	 * @param <T>
	 *            class of array items
	 * @param array
	 *            array to index
	 * @param keyPath
	 *            keyPath to index. If any object returns <code>null</code> or
	 *            NSKeyValueCoding.NullValue for this keyPath, the object is not
	 *            put into the resulting dictionary.
	 * @return a dictionary indexing the given array. If array is
	 *         <code>null</code>, an empty dictionary is returned.
	 */
	 ...
	 * the array. This method assume that the value returned for the keyPath
	 * attribute will be unique for all the objects in the array. In case of
	 * duplicate entry, if throwOnCollision is true, an exception is thrown,
	 * otherwise, the the new object will replace the previous one in the
	 * dictionary.
	 *
	 * @param array
	 *            array to index
	 ...
	 *            NSKeyValueCoding.NullValue for this keyPath, the object is not
	 *            put into the resulting dictionary.
	 * @param throwOnCollision
	 *            if <code>true</code> and two objects in the array have the
	 *            same non-null (or non-NullValue) value for keyPath, an
	 *            exception is thrown. If <code>false</code>, the last object in
	 *            the array wins.
	 * @return a dictionary indexing the given array. If array is
	 *         <code>null</code>, an empty dictionary is returned.
	 * @deprecated use
	 *             {@link #dictionaryOfObjectsIndexedByKeyPath(NSArray, String, boolean)}
	 *             instead
	 */
	@Deprecated
	 ...
	 *            array to process
	 * @param aClass
	 *            class to use. null results in the result being a copy of the
	 *            <code>array</code>.
	 * @return an array which is a subset of the <code>array</code> where each
	 *         object in the result is an instance of <code>aClass</code>.
	 */
	...
	/**
	 * Swaps the two objects at the given indexes in the given {@link NSArray}
	 * and returns a new {@link NSArray}.
	 * 
	 * @author edgar - Jan 7, 2008
	 * @param array
	 *            in that the two {@link Object}s at the given indexes have to
	 *            be swapped
	 * @param indexOfObject1
	 *            index of one object in the {@link NSArray} that will be
	 *            swapped
	 * @param indexOfObject2
	 *            index of the other object in the {@link NSArray} that will be
	 *            swapped
	 * 
	 * @return the new {@link NSArray} with the swapped elements
	 * 
	 * @throws RuntimeException
	 *             if one of the indexes is out of bound
	 ...
	 * 
	 * @throws RuntimeException
	 *             if one or both indexes are out of bounds
	 * @deprecated use {@link #swapObjects(NSMutableArray, Object, Object)}
	 *             instead
	 */
	@Deprecated
	...
	/**
	 * Swaps the object a with the object at the given index in an array
	 * inplace.
	 * 
	 * @param <T>
	 *            class of array elements
	...
	/**
	 * Swaps the object a with the object at the given index in an array and
	...
	/**
	 * <div class="en"> Check if an array is null or empty. </div>
	 * 
	 * <div class="ja"> 配列が null か空かをチェックします </div>
	 * 
	 * @param array
	 *            <div class="en">an array</div> <div class="ja">文字列配列</div>
	...
	/**
	 * <div class="en"> To create oneLine Log for an NSArray&lt;String&gt;
	 * </div>
	 * 
	 * <div class="ja"> NSArray 配列をログとして出力する時に複数行に渡らないで、一行で収まるように </div>
	 * 
	 * @param array
	 *            <div class="en">an array</div> <div class="ja">文字列配列</div>
