...
public class ERXRuntimeUtilities {
	...
	/**
	 * Hack to create a bundle after the app is loaded. Useful for the
	 * insistence of EOF on JavaXXXAdaptor bundles.
	 * 
	 * @param name
	 ...
	 * 
	 * @param bundleFile
	 *            - the directory or archive (e.g., jar, war) of the bundle to
	 *            load
	 ...
	 *            can be null
	 * @param dir
	 *            a <code>File</code> object representing the working directory,
	 *            can be null
	 * 
	 ...
	 *            any <code>long</code> number larger than <code>0</code> which
	 *            means if the process does not exit after <code>timeout</code>
	 *            milliseconds then this method throws an ERXTimeoutException
	 * 
	 * 
	 * @return the results from the processes that were executed
	 * 
	...
	public final static Result execute(String[] command, String[] envp,
		...
		try {
			...
			// Getting stream results before freeing process resources to
			// prevent a case
			// when fast process is destroyed before stream readers read from
			// buffers.
			if (isr != null) {
				if (esr != null) {
			...
			// Checking exceptions after getting results to ensure that stream
			// readers
	...
	public static class TimeoutException extends Exception {
		/**
		 * Do I need to update serialVersionUID? See section 5.6 <cite>Type
		 * Changes Affecting Serialization</cite> on page 51 of the
		 * <a href="http://java.sun.com/j2se/1.4/pdf/serial-spec.pdf">Java
		 * Object Serialization Spec</a>
		 */
