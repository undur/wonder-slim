...
protected boolean cleanPageReplacementCacheIfNecessary(String _cacheKeyToAge) {
   ...
   */
  protected TransactionRecord pageRecordForRequestContainer(LinkedHashMap pageReplacementCache) {
    WOContext context = context();
  ...
  public WOComponent restorePageForContextID(String contextID) {
    ...
    if (pageReplacementCache != null) {
      ...
      if (pageRecord == null) {
        pageRecord = pageRecordForRequestContainer(pageReplacementCache);
        if (pageRecord != null && log.isDebugEnabled()) {
          log.debug("Restored page by container ({}) since context {} was not cached", pageRecord.key(), contextID);
        }
      }
      if (pageRecord != null) {
