...
public class ERXWOContext extends ERXAjaxContext {
	 ...
	 * keep mirroring the request. This is where RouteAction used to do the
	 * same fixup for the routed request only — a context created later by a
	 * handler a route delegates to (JavaMonitor's root handler, say) never
	 * saw it.
	 */
	@Override
	public void _setRequest(WORequest request) {
	 ...
	 * the two differ, and only the former is in the URL. The context's URL base
	 * is normalised in {@link #_setRequest(WORequest)}, so it always names the
	 * application; a context without a request falls back to the
