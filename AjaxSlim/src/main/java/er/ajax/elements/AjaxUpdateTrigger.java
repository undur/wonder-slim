...
public class AjaxUpdateTrigger extends WODynamicElement {
	...
	private static void addResolved(List<String> target, WOAssociation association, WOComponent component) {
		...
		final String resolved = AjaxUpdateProtocol.updateContainerID(association.valueInComponent(component));
		if (resolved == null) {
			return;
		}
		for (String id : resolved.split(AjaxUpdateProtocol.MULTI_UPDATE_SEPARATOR)) {
